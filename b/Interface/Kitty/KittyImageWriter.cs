...
namespace b.Interface.Kitty
{
    public static class KittyImageWriter
    {
        ...
        public static void WriteImageStream(Stream? f)
        {
            WindowUtil.Update();
            ...
            foreach (byte[] payloadBuffer in f.GetBase64ByteEnumerable(4096)) // asserts it exists
            {
                ...
                if (first)
                {
                    first = false;
                    stdout.Write(
                        Encoding.UTF8.GetBytes("m=1,a=T,f=100;")); // headers for graphics protocol first payload
                }
                else
                {
            ...
            stdout.Write(Encoding.UTF8.GetBytes("_G"));
            if (!flushed)
                stdout.Write(Encoding.UTF8.GetBytes("a=T,f=100,m=0;")); // headers for graphics protocol first payload
            else
                stdout.Write(Encoding.UTF8.GetBytes("m=0;"));
            stdout.Write(last);
            stdout.Write(ControlKey);
            stdout.Write(Encoding.UTF8.GetBytes("\\"));
            stdout.Flush();
